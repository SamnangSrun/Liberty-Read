...
const PictureBooks = ({ showAll, setShowAll }) => {
    ...
    const fetchBooks = async () => {
      try {
        ...
        const animeBooks = data.books.filter(
          (book) => book.category?.name?.toLowerCase() === "Picture Books" && book.status === "approved"
        );
        setBooks(animeBooks);
      } catch (err) {
    ...
    <div className="px-2 sm:px-4">
      <div className="flex flex-col sm:flex-row justify-between items-start mb-4">
        <div className="mb-2 sm:mb-0">
          <h1 className="text-xl sm:text-2xl font-bold"> Picture Books</h1>
