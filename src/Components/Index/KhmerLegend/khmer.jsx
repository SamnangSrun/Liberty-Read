...
const Fitness = ({ showAll, setShowAll }) => {
    ...
    fetch(`${config.base_url_api}books`)
      ...
      .then((data) => {
        const animeBooks = data.books.filter(
          (book) =>
            book.category?.name?.toLowerCase() === "khmerlegend" &&
  ...
  const displayedBooks = showAll
    ? books
    : shuffleArray(books).slice(0, 4); // Random 4 books
  ...
  <div className="mb-2">
    <h1 className="text-sm sm:text-base font-bold">Khmer legends</h1>
    <h1 className="text-xl sm:text-3xl m-2 text-[#102249] font-bold">Khmer legends</h1>
    <p className="text-sm text-gray-500 sm:text-base">
      Recommended Khmer legends – Escape into magical worlds filled with wonder and adventure!
    </p>
  </div>
</div>
